...
test_get_gradient_classic = do
    ...
    let ls = replicate 1000 [Node (0,0) [(1,(0,0))], Node (0,0) [(1,(0,1))]]
    ...
    let n_net' =
    let n_net' = f_propagate (set_input n_net [1,1])
    --}
    let start = [ [Node (0,0) [(1,(0,0))], Node (1,0) [(1,(0,1))]]]
    let ls = replicate 10000 [Node (0,0) [(1,(0,0))], Node (0,0) [(1,(0,1))]]
    let end = [[Node (0,0) [], Node (0,0) [] ]]
    let net = Net $ start ++ ls ++ end
    let n_net = training_batch_classic net ([[1,1]],[[0,1]])  1
    print $ output_classic n_net [1,1]
    getCurrentTime >>= print
    print "hi"

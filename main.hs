...
draw_ids :: Int -> [Int] -> IO ([Int],[Int])
draw_ids nr ids = draw_ids' nr [] ids

draw_ids' :: Int -> [Int] ->  [Int] ->IO ([Int],[Int])
draw_ids' 0  found rest = return (found, rest)
draw_ids' nr found rest = do
  idx <- randomRIO (0::Int, (length rest -1))
  let (xs,f:ys) = splitAt idx rest
  draw_ids' (nr-1) (f:found) (xs ++ ys)


run_and_save_image :: [Int] -> Int -> String -> (([[Double]], [[Double]]) -> Net -> Double ->IO Net) -> Net -> Double -> IO Net
...
run_and_save_image ids times filename f  net s = do
                                                              ...
                                                              --                                 else draw_wrong_mnist_training_batch nr 10
                                                               (set, ids') <- draw_ids 10 ids
                                                               set <- draw_mnist_training_batch_by_ids set
                                                               print $ length  set
                                                               --set <- mnist_set
                                                               ...
                                                               print times
                                                               run_and_save_image  ids' (times + 1) filename f net' s
...
main = do
          ...
          wrong_test_predictions random_net >>= print
          -- update_random_net_con nr_times nr_trainings bs nr_con sample net s
          ...
          --trained_random_net <- run_and_save_image [0..59999] 0 "image_net" f  random_net  0.1
